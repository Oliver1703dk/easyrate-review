import { MessageSquare } from 'lucide-react';
import { Button } from '@easyrate/ui';
import { LANDING_PAGE_TEXT } from '@easyrate/shared';
import type { LandingPageBusiness, ReviewRating } from '@easyrate/shared';
import { StarRating } from './StarRating';

interface PositivePromptScreenProps {
  business: LandingPageBusiness;
  rating: ReviewRating;
  onExternalClick: () => void;
  onSkip: () => void;
  onInternalFeedbackClick?: (() => void) | undefined;
}

// Google "G" logo as SVG
function GoogleIcon() {
  return (
    <svg className="w-5 h-5" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
      <path
        fill="#4285F4"
        d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
      />
      <path
        fill="#34A853"
        d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
      />
      <path
        fill="#FBBC05"
        d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
      />
      <path
        fill="#EA4335"
        d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
      />
    </svg>
  );
}

export function PositivePromptScreen({
  business,
  rating,
  onExternalClick,
  onSkip,
  onInternalFeedbackClick,
}: PositivePromptScreenProps) {
  return (
    <div className="flex flex-col items-center text-center px-6 py-8 min-h-[100dvh] justify-center">
      {/* Header with rating display */}
      <div className="mb-6">
        <StarRating
          value={rating}
          onChange={() => {
            // noop - rating is read-only here
          }}
          size="sm"
          disabled
        />
      </div>

      {/* Title */}
      <h1 className="text-2xl font-bold text-foreground mb-2">
        {LANDING_PAGE_TEXT.positivePromptTitle}{' '}
        <span role="img" aria-label="party">
          🎉
        </span>
      </h1>

      {/* Subtitle */}
      <p className="text-muted-foreground mb-8">{LANDING_PAGE_TEXT.positivePromptSubtitle}</p>

      {/* Platform Buttons */}
      <div className="w-full max-w-xs flex flex-col gap-3 mb-6">
        {business.googleReviewUrl && (
          <Button
            variant="outline"
            className="w-full h-12 text-base justify-start px-4 gap-3"
            onClick={onExternalClick}
          >
            <GoogleIcon />
            <span>{LANDING_PAGE_TEXT.positivePromptGoogle}</span>
          </Button>
        )}
      </div>

      {/* Footer text */}
      <p className="text-sm text-muted-foreground mb-8">
        {LANDING_PAGE_TEXT.positivePromptFooter}{' '}
        <span role="img" aria-label="pray">
          🙏
        </span>
      </p>

      {/* Skip Button */}
      <Button variant="ghost" onClick={onSkip} className="text-muted-foreground">
        {LANDING_PAGE_TEXT.positivePromptSkip}
      </Button>

      {/* Legal requirement: Option to leave internal feedback */}
      {onInternalFeedbackClick && (
        <button
          type="button"
          onClick={onInternalFeedbackClick}
          className="mt-6 flex items-center gap-2 text-sm text-primary hover:underline"
        >
          <MessageSquare className="w-4 h-4" />
          {LANDING_PAGE_TEXT.positivePromptInternalOption}
        </button>
      )}
    </div>
  );
}
